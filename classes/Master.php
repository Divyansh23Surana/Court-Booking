...
Class Master extends DBConnection {
	...
	function save_booking() {
		...
		$date = $_POST['date'];
		$time = $_POST['time'];
		// Input time string in "hh:mm AM/PM" format
		$timeString = $time;
		$time24HourFormat = date("H:i:s", strtotime($timeString));
		$time_from=$time24HourFormat;
		$facility_id = $_POST['facility_id'];
	
		// Check for conflicting bookings
		$check = $this->conn->query("SELECT * FROM `booking_list` 
									 ...
									 AND status = 1")->num_rows;
		if ($check > 0) {
			$resp['status'] = 'failed';
			$resp['msg'] = 'The facility is not available at the selected date and time range.';
			return json_encode($resp);
			exit;
		}
	
		// Prepare SQL for saving data
		if (empty($_POST['id'])) {
			$sql = "INSERT INTO `booking_list` (`facility_id`, `date`, `time_from`,  `ref_code`, `client_id`, `status`) 
					VALUES ('{$facility_id}', '{$date}', '{$time_from}', '{$_POST['ref_code']}', '{$_POST['client_id']}', 1)";
			$save = $this->conn->query($sql);
		} else {
			$sql = "UPDATE `booking_list` 
					SET `facility_id` = '{$facility_id}', `date` = '{$date}', `time_from` = '{$time_from}'
					WHERE id = '{$_POST['id']}'";
			$save = $this->conn->query($sql);
		}
